...
userController.addNewUser = async (req, res, next) => {
  ...
  try {
    ...
    const userInDb = await models.User.create(newUser);

    res.locals.newUser = userInDb;
...
userController.login = async (req, res, next) => {
  try {
    ...
    const userInDb = await models.User.findOne({ username: username, password: password });

    console.log(userInDb);

    res.locals.userInDb = userInDb;
...
userController.addTask = async (req, res, next) => {
  ...
  try {
    ...
    //console.log('user', username, 'newTask', newTask, 'taskList', taskList);
    ...
    // taskList with new updated task inside
    const updatedTaskList = updatedUser.taskList;

    res.locals.newTaskList = updatedTaskList;
...
userController.deleteTask = async (req, res, next) => {
  ...
  try {
    ...
    const updatedTaskList = taskList.filter(task => task._id.toString() !== id);
